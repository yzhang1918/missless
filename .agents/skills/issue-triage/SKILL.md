...
3. Assign exactly one `scope:*` label and exactly one `kind:*` label.
4. Remove `needs-triage` once triage is complete.
5. Apply at most one `state:*` label:
...
6. Leave a short triage comment when the disposition is not obvious from the labels alone.
7. If closing the issue, record the reason and link any replacement issue, source issue, or merged PR.
