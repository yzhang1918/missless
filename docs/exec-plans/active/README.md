...
Store plans currently in execution.
...
- Plans that will later pass through stateful publish/final-gate/land checks must include:
  ...
  - `## Accepted Deferred Risks` when known out-of-slice risks are intentionally accepted
  - `## Work Breakdown` with `### Step N` sections
  - one `- Status: pending|in_progress|completed|blocked` line per step
- Each `## Accepted Deferred Risks` entry should link the accepted follow-up issue when available, or record an explicit defer reason.
