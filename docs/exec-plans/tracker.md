...
| FUP-0005 | Define resolved-host SSRF policy for fetch-normalize | P1 | todo | Human+Codex | `packages/core/src/source/fetch-normalize.ts`, `packages/core/src/providers/`, `docs/design-docs/system-design.md`, `docs/specs/pipeline-contracts.md` | Decide whether runtime should resolve DNS before fetch, how to handle private-address results and redirects, and how that policy composes with remote fetch providers and future fallback chains. |
| FUP-0006 | Define a stronger render-review cleanup trust model | P2 | todo | Human+Codex | `packages/core/src/review/build-review-bundle.ts`, `apps/cli/src/commands/render-review.ts`, `docs/design-docs/system-design.md`, `docs/specs/pipeline-contracts.md` | Current cleanup is fail-closed for many stale-artifact cases, but the repository still needs a deliberate contract for when runtime may delete prior review outputs without relying on forgeable local markers or leaving stale HTML behind. |
