...
- The first PR-triggered run of that new workflow failed with GitHub's `startup_failure` bootstrap behavior because the workflow had not landed on `main` yet, so the workflow was updated to trigger on `push` to `codex/**` branches as well. That lets the branch produce its own check context before merge.
- The first push-triggered run also failed at workflow startup because the repository Actions policy is `allowed_actions=local_only`, which rejects `actions/checkout`. The workflow was then rewritten to use a pure-shell authenticated `git fetch` checkout so it stays compatible with the repository's current Actions policy.
