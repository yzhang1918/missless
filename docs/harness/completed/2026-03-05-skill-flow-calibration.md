...
- Historical task focus: TASK-0004
...
- In scope:
  ...
  - Update repository planning records with this calibration task and deferred context for `TASK-0001`.
- Out of scope:
...
- [x] Repository planning records include `TASK-0004` and record deferred context for `TASK-0001`.

## Work Breakdown

1. Add task-intake and workflow gates in top-level docs.
2. Refine discovery/plan skill contracts and interface metadata.
3. Sync planning records and run term-level validation checks.
...
- `rg -n "TASK-0004|TASK-0001" docs/harness/completed/2026-03-05-skill-flow-calibration.md`
...
- Executed `rg -n "TASK-0004|TASK-0001" docs/harness/completed/2026-03-05-skill-flow-calibration.md`; the archived-plan references remained aligned at the time of the check.
...
- Delivered:
  ...
  - Recorded `TASK-0004`, then restored `TASK-0001` to `ready` after this PR scope was narrowed to skill-flow only.
...
- Planning record updates:
  - `TASK-0004` was marked `done` and linked to this completed plan at the time.
  - `TASK-0001` was kept out of this PR scope and set to `ready` at the time.
